...
podman run --arch "$ARCHITECTURE" -v "$MOUNTPOINT":/mnt:Z registry.fedoraproject.org/fedora:latest bash -c "dnf -y install --installroot /mnt --releasever 34 glibc-minimal-langpack picocom --nodocs --setopt install_weak_deps=False"
